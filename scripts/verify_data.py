...
inv_scope = inv[inv['仓库'].astype(str).str.contains('PHONES', na=False)]
# 库存口径: 仅 SMART PHONE + TABLET TYPE（SMART DEVICE=智能穿戴/音频，不属于手机周转口径）
# 分类判定必须与 process_data 完全同规则、同取值空间(含 GENERAL 行):
# 同一型号在数据源里可能被标成两种分类(如 ITEL IT5363 既标 FEATURE PHONE 又标 SMART PHONE),
# 按型号取众数(批次数最多), 平票取可卖数合计大者, 再平票按 smart>tablet>feature 优先 —— 型号只有一个分类
_CAT_LEVEL = {'SMART PHONE': 'smart', 'FEATURE PHONE': 'feature', 'TABLET TYPE': 'tablet'}
_CAT_PRIORITY = {'smart': 0, 'tablet': 1, 'feature': 2, 'other': 3}
inv_scope = inv_scope.copy()
inv_scope['_cat_raw'] = inv_scope['二级分类名称'].astype(str).str.strip()
_cc = inv_scope.groupby(['商品型号', '_cat_raw'], as_index=False).agg(_n=('可卖数', 'size'), _q=('可卖数', 'sum'))
_cc['_cat'] = _cc['_cat_raw'].map(_CAT_LEVEL).fillna('other')
_cc['_pri'] = _cc['_cat'].map(_CAT_PRIORITY)
_canon = (_cc.sort_values(['_n', '_q', '_pri'], ascending=[False, False, True])
             .drop_duplicates('商品型号').set_index('商品型号')['_cat'])
inv_scope['_main_cat'] = inv_scope['商品型号'].map(_canon).fillna('other')
# 汇总用集合: 排除总仓(GENERAL), 仅 smart+tablet
inv_st = inv_scope[
    inv_scope['_main_cat'].isin(['smart', 'tablet']) &
    ~inv_scope['仓库'].astype(str).str.contains('GENERAL', na=False)
].copy()
...
if pv:
    ...
    # 独立构建「型号 × 营业日」实际成交均价, 用于复现调价生效日的判定
    _sd = s.copy()
    _sd['_m'] = _sd['型号'].astype(str).map(_nk)
    ...
    _avg_by = {}
    for (_mmk, _ddk), _g in _sd.groupby(['_m', '_d']):
        _qq = _g['_q'].sum()
        if _qq > 0:
            _avg_by.setdefault(_mmk, {})[str(_ddk)] = _g['_r'].sum() / _qq

    _bad, _checked, _split_bad = [], 0, []
    for r in pv.get('detail', []):
        ...
        # ① 优先按销售成交价穿越"旧价/新价中点"的那一天定位生效日
        sp = None
        _cur, _cmp = r.get('cur_price'), r.get('cmp_price')
        if _cur is not None and _cmp is not None and _cur != _cmp:
            _mid = (float(_cur) + float(_cmp)) / 2.0
            _up = float(_cur) > float(_cmp)
            _dayq = _avg_by.get(_t, {})
            for _i, _d in enumerate(vd):
                _a = _dayq.get(str(_d))
                if _a is not None and ((_a >= _mid) if _up else (_a <= _mid)):
                    sp = _i
                    break
        # ② 退回价格表签发日 ③ 退回最后2个营业日
        if sp is None:
        ...
        if not pre or not post:
            continue
        _checked += 1
        if r.get('split_date') is not None and sp < len(vd) and r.get('split_date') != vd[sp]:
            _split_bad.append((r['model'], r.get('split_date'), vd[sp]))
    ...
    if _bad:
        ...
        ok.append(f"  OK   调价前/后日均窗口重算一致 ({_checked} 个机型, 生效日按销售成交价定位/回退 {_eff}, 已剔除未来补0营业日)")
    if _split_bad:
        fail.append(f"  FAIL 记录的生调价生效日与重算不符 {len(_split_bad)} 个: {_split_bad[:5]}")
    else:
        ok.append(f"  OK   调价生效日(split_date)与销售成交价重算一致")
