...
# IGANDO / MSL MUSHIN2 已关店: 从当期/对比销售、目标配件sheet、门店映射中剔除
# 注: 目标文件"手机"sheet已由生成脚本物理删除这两行
CLOSED_DEPTS = {'D_IGANDO-IKOTUN ROAD-LAGOS', 'D_MSL MUSHIN2-ISOLO ROAD-LAGOS'}
for _df in (june, may, tgt_acc):
    if '销售部门' in _df.columns:
        _df.drop(_df[_df['销售部门'].isin(CLOSED_DEPTS)].index, inplace=True)
...
# 关店门店仓库剔除 (关店后残余库存不再作为门店库存展示)
inv = inv[~inv['仓库'].astype(str).str.contains(r'MUSHIN2-PHONES|IGANDO-PHONES', na=False)]
...
try:
    ...
    # 关店门店从权威映射中移除
    for d in CLOSED_DEPTS:
        DEPT_TO_WAREHOUSE.pop(d, None)
...
# Categorize —— 同一型号必须只有一个分类
# 数据源里个别型号在不同仓库(甚至同一仓库的不同批次行)被标成两种分类,
# 例如 ITEL IT5363 既标 FEATURE PHONE 又标 SMART PHONE。
# 若按 (仓库,型号) 取"第一行"的 cat_type, 同一型号会在不同仓库分别计入不同口径,
# 造成智能机/功能机库存口径漂移(实测 4 台)。
# 统一规则: 按型号取众数(出现的批次数最多), 平票取可卖数合计更大者, 再平票按 smart>tablet>feature 优先。
_CAT_LEVEL = {'SMART PHONE': 'smart', 'FEATURE PHONE': 'feature', 'TABLET TYPE': 'tablet'}
_CAT_PRIORITY = {'smart': 0, 'tablet': 1, 'feature': 2, 'other': 3}
inv_phones['_cat_raw'] = inv_phones['二级分类名称'].astype(str).str.strip()
_cat_cnt = inv_phones.groupby(['商品型号', '_cat_raw'], as_index=False).agg(
    _n=('可卖数', 'size'), _q=('可卖数', 'sum'))
_cat_cnt['_cat'] = _cat_cnt['_cat_raw'].map(_CAT_LEVEL).fillna('other')
_cat_cnt['_pri'] = _cat_cnt['_cat'].map(_CAT_PRIORITY)
_canon_cat = (_cat_cnt.sort_values(['_n', '_q', '_pri'], ascending=[False, False, True])
                      .drop_duplicates('商品型号')
                      .set_index('商品型号')['_cat'])
inv_phones['cat_type'] = inv_phones['商品型号'].map(_canon_cat).fillna('other')
_cat_amb = _cat_cnt.groupby('商品型号').size()
_cat_amb = _cat_amb[_cat_amb > 1].index.tolist()
if _cat_amb:
    print(f"  Warning: {len(_cat_amb)} 型号分类不唯一(数据源问题), 已按型号众数统一判定:")
    for _m in _cat_amb:
        _raw = _cat_cnt[_cat_cnt['商品型号'] == _m]
        _detail = ", ".join(f"{r['_cat_raw']}={int(r['_q'])}台/{int(r['_n'])}批"
                            for _, r in _raw.sort_values('_q', ascending=False).iterrows())
        print(f"    - {_m}: 判定={_canon_cat.get(_m)} (来源: {_detail})")
...
if args.price_list:
    ...
    # 价格表偶尔对同一型号保留"旧价行 + 新价行"(如 14.09 的 GALAXY A07-A075F(128+4) 同时有
    # 209,600 与 250,300)。原先 price_map[mdl_ns] = ... 是"最后一行胜出", 会静默取到尚未执行的新价。
    # 规则: 解析价有冲突时, 用**当月销售实际成交均价**挑最贴近的一行(价格以市场实际为准);
    #       没有销量或无法判定时, 保留最后一行并打印告警。
    _sales_price_by_model = {}
    try:
        _sp = june.groupby(june['型号'].astype(str).str.strip().str.upper()).agg(
            _q=('销售数量', 'sum'), _r=('零售金额', 'sum'))
        for _m, _row in _sp.iterrows():
            if _row['_q'] and _row['_q'] > 0:
                _sales_price_by_model[str(_m).replace(' ', '')] = _row['_r'] / _row['_q']
    except Exception as _e:
        print(f"  [warn] 销售成交均价索引构建失败, 冲突行将退化为取最后一行: {_e}")

    _PRICE_CONFLICTS = []
    _CONFLICT_REPORTED = set()
    _CONFLICT_PICK = {}

    def select_conflict_rows(df, tag):
        """返回 {mdl_ns: row_index}, 同型号多行且价格冲突时按销售成交均价择一。"""
        buckets = {}
        for _i, _r in df.iterrows():
            _mdl_ns = str(_r['SYSTEM MODEL NAME']).strip().upper().replace(' ', '')
            _p, _s = resolve_price(_r)
            buckets.setdefault(_mdl_ns, []).append((_i, _p, _s))
        chosen = {}
        for _mdl_ns, _cands in buckets.items():
            if len(_cands) == 1:
                chosen[_mdl_ns] = _cands[0][0]
                continue
            _vals = {round(float(p), 2) for _, p, _ in _cands if not pd.isna(p)}
            if len(_vals) <= 1:
                chosen[_mdl_ns] = _cands[-1][0]      # 价格一致, 无冲突
                continue
            if (tag, _mdl_ns) in _CONFLICT_REPORTED:
                chosen[_mdl_ns] = _CONFLICT_PICK[(tag, _mdl_ns)]   # 同一份表已判定过, 复用同一结论
                continue
            _CONFLICT_REPORTED.add((tag, _mdl_ns))
            _act = _sales_price_by_model.get(_mdl_ns)
            if _act:
                _pick = min(_cands, key=lambda c: abs(float(c[1]) - _act))
                _act_note = f"销售成交均价 {_act:,.0f}"
            else:
                _pick = _cands[-1]
                _act_note = "该型号无销量, 无法判定"
            chosen[_mdl_ns] = _pick[0]
            _CONFLICT_PICK[(tag, _mdl_ns)] = _pick[0]
            _PRICE_CONFLICTS.append({
                'model': _mdl_ns,
                'candidates': [round(float(p), 0) for _, p, _ in _cands],
                'picked': round(float(_pick[1]), 0),
                'basis': _act_note,
            })
            print(f"  [价格表冲突] {_mdl_ns} ({tag}): 候选 {[round(float(p),0) for _,p,_ in _cands]} → "
                  f"取 {round(float(_pick[1]),0)} ({_act_note})")
        return chosen
    ...
    for mdl_ns, _ri in select_conflict_rows(pl, '当前').items():
        r = pl.loc[_ri]
...
# ---- Brand-safe price matching helpers (fix: partial match must never cross brands) ----
# 销售数据里的品牌是权威口径；价格表 BRANDS 只用于校验/兜底，绝不覆盖销售品牌。
# 历史 bug: "A6(256+8)" 被子串匹配到 TECNO "CAMON30S-PRO-CLA6(256+8)"，
# 导致 OPPO A6(256+8) 品牌变成 TECNO、价格取成 276100(应为 361900)，型号分析里 OPPO 筛不到该机型。
BRAND_ALIAS = {'APPLE': 'IPHONE', 'IPHONE': 'APPLE'}

def brand_compat(b1, b2):
    """两个品牌名是否相容（相同 / 别名 / 任一方为空）"""
    b1 = str(b1 or '').strip().upper()
    b2 = str(b2 or '').strip().upper()
    if not b1 or not b2:
        return True          # 一方缺失时不构成冲突
    return b1 == b2 or BRAND_ALIAS.get(b1) == b2

def match_price_safe(mdl_ns, sales_brand):
    """安全匹配价格表。
    返回 (price, source, pl_brand)；匹配不到返回 (nan, '', '')。
    匹配优先级:
      1) 裸型号精确
      2) BRAND-型号 精确（价格表多为 "OPPO-A6(256+8)" 形式）
      3) 品牌相容前提下的部分匹配，取"长度最短的包含者"(最相似)，而非第一个
      4) 品牌冲突时一律不采用宁缺勿错
    """
    if not price_map:
        return float('nan'), '', ''
    sb = str(sales_brand or '').strip().upper()
    # 1) 精确：裸型号
    if mdl_ns in price_map:
        p, src = price_map[mdl_ns]
        return p, src, brand_map.get(mdl_ns, '')
    # 2) 精确：品牌前缀
    for cand in (f'{sb}-{mdl_ns}', f'{BRAND_ALIAS.get(sb, "")}-{mdl_ns}'):
        if cand and cand in price_map:
            p, src = price_map[cand]
            return p, src, brand_map.get(cand, '')
    # 3) 部分匹配：品牌必须相容，取长度最短(最贴近)的候选
    best = None
    for pl_ns, (p, src) in price_map.items():
        if not (mdl_ns in pl_ns or pl_ns in mdl_ns):
            continue
        pb = str(brand_map.get(pl_ns, '') or '').strip().upper()
        if not brand_compat(sb, pb):
            continue          # 品牌冲突 → 跳过，防止 TECNO 型号污染 OPPO
        if best is None or len(pl_ns) < len(best[0]):
            best = (pl_ns, p, src, pb)
    if best:
        return best[1], best[2] + ' (partial match)', best[3]
    return float('nan'), '', ''
...
for _, r in model_stats.iterrows():
    ...
    # Match price (brand-safe: 品牌以销售数据为准，价格表品牌仅校验)
    p_matched, src_matched, pl_brand = match_price_safe(mdl_nospace, matched_brand)
    if src_matched:
        price = p_matched
        price_source = src_matched
        price_tier = get_price_tier(price)
        # 仅当销售数据无品牌时才回退到价格表品牌（绝不覆盖销售品牌）
        if not matched_brand and pl_brand:
            matched_brand = pl_brand
    ...
    # 品牌防污染: 模糊匹配可能命中其它品牌的相似型号(如 OPPO A6(256+8) 命中
    # TECNO CAMON30S-PRO-CLA6(256+8)), 命中集合若含多品牌则只保留与销售品牌相容的行
    if not inv_model.empty and '品牌' in inv_model.columns and matched_brand:
        bmask = inv_model['品牌'].apply(lambda b: brand_compat(matched_brand, b))
        if bmask.any() and not bmask.all():
            inv_model = inv_model[bmask]
    ...
    m10_model_analysis.append({
        ...
        'price': int(price) if pd.notna(price) and price > 0 else 0,
...
if args.price_list and args.price_list_compare:
    ...
    cmp_price_map = {}
    cmp_brand_map = {}
    for mdl_ns, _ri in select_conflict_rows(pl_cmp, '对比').items():
        r = pl_cmp.loc[_ri]
        price, src = resolve_price(r)
    ...
    # 型号 × 营业日 的实际成交价索引 (用于定位调价真正落地的日期, 而不是用价格表签发日)
    _sales_daily_by_model = {}
    try:
        _sd = june.copy()
        _sd['_m'] = _sd['型号'].astype(str).map(norm_key)
        _sd['_d'] = _sd['记账日期'].astype(str).str.slice(5, 10)
        _sd['_q'] = pd.to_numeric(_sd['销售数量'], errors='coerce').fillna(0)
        _sd['_r'] = pd.to_numeric(_sd['零售金额'], errors='coerce').fillna(0)
        for (_mm, _dd), _grp in _sd.groupby(['_m', '_d']):
            _sales_daily_by_model.setdefault(_mm, {})[str(_dd)] = (
                float(_grp['_q'].sum()), float(_grp['_r'].sum()))
    except Exception as _e:
        print(f"  [warn] 销售日成交价索引构建失败, 调价生效日退回价格表日期: {_e}")
    ...
    for cur_ns, cur_info in cur_full.items():
        if cur_ns in matched_pairs:
            ...
            # 调价生效日切分: 调价前日均 vs 调价后日均(样本小, 仅作初步跟踪)
            pre_daily_avg = post_daily_avg = post_vs_pre = None
            pre_days = post_days = None
            split_date = None
            if price_changed and m11_match:
                _jdates = m11_match['june_dates']
                _jdaily = m11_match['june_daily']
                # 月趋势数组覆盖全月, 未来营业日补 0 —— 必须先裁到已发生的营业日
                _today_mmdd = today_str[5:10]
                _valid = len(_jdates)
                for _i, _d in enumerate(_jdates):
                    if str(_d) > _today_mmdd:
                        _valid = _i
                        break
                _vd, _vq = _jdates[:_valid], _jdaily[:_valid]
                _split = None
                # ① 优先用**销售实际成交价**定位生效日: 价格表签发日只是"发单日",
                #    实际调价往往更早落地(如 REDMI A7PRO(64+4) 表上 14.09 才给新价, 门店 09-08 就按新价卖了)。
                try:
                    _dayq = _sales_daily_by_model.get(norm_key(model_short)) or {}
                    _mid = (float(cmp_price) + float(cur_price)) / 2.0
                    _up = float(cur_price) > float(cmp_price)
                    for _i, _d in enumerate(_vd):
                        _q, _amt = _dayq.get(str(_d), (0.0, 0.0))
                        if _q > 0:
                            _avg = _amt / _q
                            if (_avg >= _mid) if _up else (_avg <= _mid):
                                _split = _i
                                break
                except Exception:
                    _split = None
                ...
                if _split is None:
                    try:
                        _cd = _extract_price_date(args.price_list)  # 例: 2026年9月8日
                        _mmdd = "%02d-%02d" % (int(_cd.split('年')[1].split('月')[0]),
                                               int(_cd.split('月')[1].split('日')[0]))
                        if _mmdd in _vd:
                            _split = _vd.index(_mmdd)
                    except Exception:
                        _split = None
                if _split is None:
                    _split = max(1, len(_vd) - 2)  # 回退: 默认最后2个营业日为调价后
                _pre, _post = _vq[:_split], _vq[_split:]
                split_date = _vd[_split] if _split < len(_vd) else None
                if _pre and _post:
                    pre_daily_avg = round(sum(_pre) / len(_pre), 1)
                    post_daily_avg = round(sum(_post) / len(_post), 1)
                    pre_days, post_days = len(_pre), len(_post)
                    if pre_daily_avg > 0:
                        post_vs_pre = round((post_daily_avg - pre_daily_avg) / pre_daily_avg * 100, 1)
    ...
    price_summary = {
        ...
        # 同型号多行且价格冲突时的取舍记录(按销售成交均价择一)
        'price_row_conflicts': _PRICE_CONFLICTS,
    }
...
# ========== MODULE 13: DIRECTOR COCKPIT ANALYTICS ==========
print("Module 13: Director cockpit analytics...")

# ---- 13.1 月底达成预测 ----
_daily_smart = [float(r['smart_qty']) for r in company_daily]
_done = float(june_smart_qty)
_tgt = float(TOTAL_TARGET)
_rem = float(REMAINING_DAYS)

def _seg_avg(seq, n):
    seg = seq[-n:] if len(seq) >= n else seq
    return (sum(seg) / len(seg)) if seg else 0.0

def _scenario(label, avg):
    proj = _done + avg * _rem
    return {
        'label': label,
        'daily_avg': round(avg, 1),
        'proj_qty': round(proj, 0),
        'proj_rate': round(proj / _tgt * 100, 1) if _tgt else 0,
        'gap': round(max(0.0, _tgt - proj), 0),
    }

_avg3 = _seg_avg(_daily_smart, 3)
_avg5 = _seg_avg(_daily_smart, 5)
_avg_all = (_done / ELAPSED_DAYS) if ELAPSED_DAYS else 0
_scen = [_scenario('近3日趋势', _avg3), _scenario('近5日趋势', _avg5), _scenario('全期日均', _avg_all)]
_conservative = min(s['proj_rate'] for s in _scen[:2]) if _scen else 0
if _conservative >= 100:
    _risk = {'level': '低风险', 'color': '#22c55e', 'desc': '按当前动能可完成月度目标'}
elif _conservative >= 97:
    _risk = {'level': '中风险', 'color': '#f59e0b', 'desc': '略有缺口，需保持并小幅提速'}
elif _conservative >= 93:
    _risk = {'level': '高风险', 'color': '#f97316', 'desc': '缺口明显，需显著提速'}
else:
    _risk = {'level': '严重风险', 'color': '#ef4444', 'desc': '按当前动能无法完成，需立即干预'}

_slope = 0.0
if len(_daily_smart) >= 2:
    _ys = _daily_smart[-5:]
    _xs = list(range(len(_ys)))
    _n = len(_ys)
    _den = _n * sum(x * x for x in _xs) - sum(_xs) ** 2
    if _den:
        _slope = round((_n * sum(x * y for x, y in zip(_xs, _ys)) - sum(_xs) * sum(_ys)) / _den, 1)

_progress_diff = round((_done / _tgt - time_progress) * 100, 1) if _tgt else 0
if _progress_diff >= 2:
    _plight = {'level': '绿', 'color': '#22c55e', 'desc': '领先时间进度'}
elif _progress_diff > -2:
    _plight = {'level': '黄', 'color': '#f59e0b', 'desc': '与时间进度基本持平'}
else:
    _plight = {'level': '红', 'color': '#ef4444', 'desc': '落后时间进度'}

_forecast = {
    'done': round(_done, 0),
    'target': round(_tgt, 0),
    'remaining_days': int(_rem),
    'elapsed_days': int(ELAPSED_DAYS),
    'completion_rate': round(_done / _tgt * 100, 1) if _tgt else 0,
    'time_progress': round(time_progress * 100, 1),
    'progress_diff': _progress_diff,
    'progress_light': _plight,
    'daily_needed': round(daily_needed_total, 1),
    'daily_avg_now': round(_avg3, 1),
    'needed_uplift_pct': round((daily_needed_total - _avg3) / _avg3 * 100, 1) if _avg3 > 0 else None,
    'trend_slope': _slope,
    'trend_dir': '下行' if _slope < -8 else ('上行' if _slope > 8 else '平稳'),
    'scenarios': _scen,
    'risk': _risk,
    'dates': [r['date'] for r in company_daily],
    'daily_series': _daily_smart,
}

# ---- 13.2 库存资金占用 + 库龄结构 ----
_inv_v = inv_turnover.copy()
_inv_v['_qty'] = pd.to_numeric(_inv_v['可卖数'], errors='coerce').fillna(0)
_inv_v['_price'] = pd.to_numeric(_inv_v['平均单价'], errors='coerce').fillna(0)
_inv_v['_val'] = _inv_v['_qty'] * _inv_v['_price']
_inv_v['_age'] = pd.to_numeric(_inv_v['门店库龄'], errors='coerce').fillna(0)
_total_fund = float(_inv_v['_val'].sum())
_total_inv_stock = float(_inv_v['_qty'].sum())

_brand_fund = _inv_v.groupby('品牌').agg(qty=('_qty', 'sum'), fund=('_val', 'sum')).reset_index()
_brand_fund = _brand_fund.merge(brand_turnover_df[['品牌', 'turnover_days']], on='品牌', how='left')
_brand_fund['turnover_days'] = _brand_fund['turnover_days'].fillna(9999)
_brand_fund['avg_price'] = (_brand_fund['fund'] / _brand_fund['qty'].where(_brand_fund['qty'] > 0, 1)).round(0)
_brand_fund['fund_pct'] = (_brand_fund['fund'] / _total_fund * 100).round(1) if _total_fund else 0
_brand_fund = _brand_fund.sort_values('fund', ascending=False)

_age_bins = [-1, 30, 60, 90, 180, 10 ** 9]
_age_labels = ['≤30天', '31-60天', '61-90天', '91-180天', '>180天']
_inv_v['_ageb'] = pd.cut(_inv_v['_age'], bins=_age_bins, labels=_age_labels)
_age_agg = _inv_v.groupby('_ageb', observed=False).agg(qty=('_qty', 'sum'), fund=('_val', 'sum')).reset_index()
_age_rows = [{'bucket': str(r['_ageb']), 'qty': float(r['qty']), 'fund': float(r['fund']),
              'fund_pct': round(float(r['fund']) / _total_fund * 100, 1) if _total_fund else 0}
             for _, r in _age_agg.iterrows()]
_age_old = [r for r in _age_rows if r['bucket'] in ('91-180天', '>180天')]

_store_fund_rows = []
for _s in sorted(inv_store_agg, key=lambda x: -(x.get('fund_tied') or 0))[:10]:
    _td = _s.get('turnover_days')
    _store_fund_rows.append({
        'store': _s.get('store_short') or _s.get('inv_store'),
        'dept': _s.get('inv_store'),
        'fund': float(_s.get('fund_tied') or 0),
        'stock': float(_s.get('total_stock') or 0),
        'turnover_days': (round(float(_td), 1) if _td is not None and float(_td) < 9999 else None),
    })

_inv_value = {
    'total_fund': round(_total_fund, 0),
    'total_stock': round(_total_inv_stock, 0),
    'brands': _brand_fund.to_dict('records'),
    'aging': _age_rows,
    'aging_over90_fund': round(sum(r['fund'] for r in _age_old), 0),
    'aging_over90_qty': round(sum(r['qty'] for r in _age_old), 0),
    'aging_over90_pct': round(sum(r['fund'] for r in _age_old) / _total_fund * 100, 1) if _total_fund else 0,
    'top_stores': _store_fund_rows,
}

# ---- 13.3 价量联动归因 ----
_pv_changed = [c for c in price_analysis if c.get('price_changed')]

def _pv_agg(rows):
    pcts = [c['pct'] for c in rows if c.get('pct') is not None]
    sales = [c['post_vs_pre'] for c in rows if c.get('post_vs_pre') is not None]
    mom = [c['mom_sales_change'] for c in rows if c.get('mom_sales_change') is not None]
    return {
        'count': len(rows),
        'avg_price_pct': round(sum(pcts) / len(pcts), 1) if pcts else None,
        'avg_post_vs_pre': round(sum(sales) / len(sales), 1) if sales else None,
        'sample_post': len(sales),
        'avg_mom': round(sum(mom) / len(mom), 1) if mom else None,
        'total_qty': int(sum((c.get('june_sales_qty') or 0) for c in rows)),
    }

# 各机型调价落地日可能不同(按销售实际成交价定位), 取中位数作为大盘基准的对齐窗口
_pv_post_list = sorted(c.get('post_days') for c in _pv_changed if c.get('post_days'))
_pv_post_days = _pv_post_list[len(_pv_post_list) // 2] if _pv_post_list else None
_pv_split_dates = sorted({c.get('split_date') for c in _pv_changed if c.get('split_date')})
# 大盘基准: 同一时间窗口整体变化, 用于扣除市场因素(否则会把大盘下滑误读成调价效果)
_bench_post_n = int(_pv_post_days or 2)
_bench_pre = _seg_avg(_daily_smart[:-_bench_post_n], 6) if len(_daily_smart) > _bench_post_n else 0
_bench_post = _seg_avg(_daily_smart[-_bench_post_n:], _bench_post_n) if len(_daily_smart) >= _bench_post_n else 0
_benchmark_pct = round((_bench_post - _bench_pre) / _bench_pre * 100, 1) if _bench_pre else None

_pv_up = [c for c in _pv_changed if (c.get('diff') or 0) > 0]
_pv_dn = [c for c in _pv_changed if (c.get('diff') or 0) < 0]
_pv_detail = sorted(_pv_changed, key=lambda x: -(x.get('june_sales_qty') or 0))[:30]
_price_volume = {
    'increases': _pv_agg(_pv_up),
    'decreases': _pv_agg(_pv_dn),
    'post_days': _pv_post_days,
    'split_dates': _pv_split_dates,
    'benchmark_pct': _benchmark_pct,
    'detail': [{
        'model': c['model'], 'brand': c['brand'],
        'cur_price': c.get('cur_price'), 'cmp_price': c.get('cmp_price'),
        'pct': c.get('pct'), 'june_sales_qty': c.get('june_sales_qty'),
        'pre_daily_avg': c.get('pre_daily_avg'), 'post_daily_avg': c.get('post_daily_avg'),
        'post_vs_pre': c.get('post_vs_pre'), 'mom_sales_change': c.get('mom_sales_change'),
        'pre_days': c.get('pre_days'), 'post_days': c.get('post_days'),
        'split_date': c.get('split_date'),
    } for c in _pv_detail],
}

# ---- 13.4 门店健康度综合评分 (达成40% + 周转25% + 稳定20% + 人效15%) ----
_rate_map = {s['dept']: s.get('rate') for s in store_perf}
_short_map = {s['dept']: s.get('short') for s in store_perf}
_turn_map = {}
for _s in inv_store_agg:
    _td = _s.get('turnover_days')
    _turn_map[_s.get('inv_store')] = (float(_td) if _td is not None and float(_td) < 9999 else None)
_cv_map = {s.get('dept'): s.get('cv') for s in store_volatility}
_eff_map = {s.get('销售部门'): s.get('avg_smart_per_person') for s in sp_eff_records}

_keys_h = [s['dept'] for s in store_perf]
_rate_vec = [_rate_map.get(k) for k in _keys_h]
_turn_vec = [_turn_map.get(k) for k in _keys_h]
_cv_vec = [_cv_map.get(k) for k in _keys_h]
_eff_vec = [_eff_map.get(k) for k in _keys_h]

def _ranks(vals, higher_better=True):
    ...
    if n == 1:
        out[idx[0]] = 1.0
        return out
    order = sorted(idx, key=lambda i: vals[i], reverse=higher_better)
    for pos, i in enumerate(order):
        out[i] = 1.0 - pos / (n - 1)
    return out

_r_rate = _ranks(_rate_vec, True)
_r_turn = _ranks(_turn_vec, False)
_r_cv = _ranks(_cv_vec, False)
_r_eff = _ranks(_eff_vec, True)

_store_health = []
for _i, _k in enumerate(_keys_h):
    _score = 100.0 * (0.40 * _r_rate[_i] + 0.25 * _r_turn[_i] + 0.20 * _r_cv[_i] + 0.15 * _r_eff[_i])
    if _score >= 70:
        _lvl, _col = '优秀', '#22c55e'
    elif _score >= 55:
        _lvl, _col = '良好', '#3b82f6'
        ...
        _lvl, _col = '需关注', '#ef4444'
    _store_health.append({
        'dept': _k, 'short': _short_map.get(_k, _k),
        'score': round(_score, 1), 'level': _lvl, 'color': _col,
        'rate': _rate_map.get(_k), 'turnover_days': _turn_map.get(_k),
        'cv': _cv_map.get(_k), 'eff': _eff_map.get(_k),
        'staff_count': next((x.get('staff_count') for x in sp_eff_records if x.get('销售部门') == _k), None),
    })
_store_health.sort(key=lambda x: -x['score'])
_health_summary = {
    'excellent': sum(1 for s in _store_health if s['level'] == '优秀'),
    'good': sum(1 for s in _store_health if s['level'] == '良好'),
    'normal': sum(1 for s in _store_health if s['level'] == '一般'),
    'attention': sum(1 for s in _store_health if s['level'] == '需关注'),
    'stores': _store_health,
}
print(f"  Health: {_health_summary['excellent']} excellent / {_health_summary['attention']} need attention")

# ---- 13.5 缺货损失估算 (假设补货周期7天) ----
_REPLENISH_DAYS = 7.0
_so_rows = []
for _m in model_turnover:
    _td = _m.get('turnover_days')
    _da = float(_m.get('daily_avg_sales') or 0)
    if _td is None or float(_td) >= _REPLENISH_DAYS or _da <= 0:
        continue
    _short_days = _REPLENISH_DAYS - float(_td)
    _lost_days = min(_short_days * (_rem / _REPLENISH_DAYS if _REPLENISH_DAYS else 1), _rem)
    _lost_units = _lost_days * _da
    _price = float(_m.get('avg_unit_price') or 0)
    _so_rows.append({
        'brand': _m.get('品牌'), 'model': _m.get('商品型号'),
        'turnover_days': round(float(_td), 1), 'daily_avg_sales': round(_da, 1),
        'short_days': round(_short_days, 1), 'lost_units': round(_lost_units, 0),
        'lost_revenue': round(_lost_units * _price, 0),
    })
_so_rows.sort(key=lambda x: -x['lost_revenue'])
_so_brand = {}
for _r in _so_rows:
    _b = _so_brand.setdefault(_r['brand'], {'brand': _r['brand'], 'lost_units': 0.0, 'lost_revenue': 0.0, 'models': 0})
    _b['lost_units'] += _r['lost_units']
    _b['lost_revenue'] += _r['lost_revenue']
    _b['models'] += 1
_stockout = {
    'assumption_days': _REPLENISH_DAYS,
    'total_lost_units': round(sum(r['lost_units'] for r in _so_rows), 0),
    'total_lost_revenue': round(sum(r['lost_revenue'] for r in _so_rows), 0),
    'model_count': len(_so_rows),
    'top_models': _so_rows[:15],
    'brands': sorted(_so_brand.values(), key=lambda x: -x['lost_revenue']),
}
print(f"  Stockout estimate: {_stockout['model_count']} models, lost {_stockout['total_lost_units']:.0f} units")

# ---- 13.6 品牌毛利结构象限 (销量 x 毛利率) ----
_bm = june_brands[['品牌', 'qty', 'revenue', 'profit', 'profit_rate']].copy()
# 用均值而非中位数切分: 中位数会把多个品牌正好压在线上的情况归入"双优", 失去区分度
_med_q = float(_bm['qty'].mean())
_med_p = float(_bm['profit_rate'].mean())
_bm_rows = []
for _, _r in _bm.iterrows():
    _q = float(_r['qty'])
    _p = float(_r['profit_rate'])
    if _q >= _med_q and _p >= _med_p:
        _quad, _qc = '双优（明星）', '#22c55e'
    elif _q >= _med_q and _p < _med_p:
        _quad, _qc = '走量低利', '#3b82f6'
    elif _q < _med_q and _p >= _med_p:
        _quad, _qc = '高利小众', '#8b5cf6'
    else:
        _quad, _qc = '双低（调整）', '#ef4444'
    _bm_rows.append({
        'brand': _r['品牌'], 'qty': _q, 'revenue': float(_r['revenue']),
        'profit': float(_r['profit']), 'profit_rate': _p,
        'quadrant': _quad, 'color': _qc,
    })
_brand_matrix = {
    'qty_cut': round(_med_q, 0),
    'margin_cut': round(_med_p, 1),
    'total_qty': round(float(_bm['qty'].sum()), 0),
    'brands': sorted(_bm_rows, key=lambda x: -x['qty']),
}

# ========== MODULE 14: AI INSIGHTS (交叉关联分析引擎) ==========
# 在 m13~m18 各模块结果之上做跨模块关联推理，产出可读的经营洞察。
# 每条 insight = { id, tag, severity, title, finding, evidence[], action }
# 不是罗列数字，而是回答"发生了什么、为什么、该做什么"。
print("Module 14: AI insights engine...")

def _ins(_id, _tag, _sev, _title, _finding, _evidence, _action):
    return {'id': _id, 'tag': _tag, 'severity': _sev, 'title': _title,
            'finding': _finding, 'evidence': _evidence, 'action': _action}
...
# ---- 14.1 趋势动能与目标缺口 ----
if _progress_diff <= -2:
    _ai_insights.append(_ins(
        'trend_gap', '目标达成', _SEV_H,
        f'落后时间进度 {abs(_progress_diff):.1f} 个百分点',
        f'时间已过 {time_progress*100:.1f}%，智能机完成 {_forecast["completion_rate"]:.1f}%，进度差 {_progress_diff:+.1f}pp。'
        f'近 3 日日均 {_avg3:.0f} 台，远低于剩余每日所需 {daily_needed_total:.0f} 台，需再提速 {_forecast["needed_uplift_pct"]}%。',
        [f'近3日口径月底预测 {_scen[0]["proj_rate"]:.1f}%（缺 {_scen[0]["gap"]:.0f} 台）',
         f'近5日口径月底预测 {_scen[1]["proj_rate"]:.1f}%（缺 {_scen[1]["gap"]:.0f} 台）',
         f'全期口径月底预测 {_scen[2]["proj_rate"]:.1f}%（缺 {_scen[2]["gap"]:.0f} 台）',
         f'近5日趋势斜率 {_slope:+.1f} 台/日（{_forecast["trend_dir"]}）'],
        '把「剩余每日 1,260 台」设为后半程唯一盯办目标，按门店拆解到人，落后门店每日复盘。'))

# ---- 14.2 库存与动销方向背离 ----
_brand_inv_map = {str(b['品牌']).strip().upper(): b for b in _inv_value.get('brands', [])}
_brand_mom_map = {str(b['品牌']).strip().upper(): b.get('mom_pct', 0) for b in june_brands.to_dict('records')}
_declining_overstocked = []
for _br in _brand_matrix.get('brands', []):
    _bn = str(_br['brand']).strip().upper()
    _mom = _brand_mom_map.get(_bn, 0)
    if _mom <= -12 and _bn in _brand_inv_map:
        _inv_b = _brand_inv_map[_bn]
        _declining_overstocked.append((_br['brand'], _mom, _inv_b.get('fund', 0)))
if _declining_overstocked:
    _declining_overstocked.sort(key=lambda x: -x[2])
    _dos_top = _declining_overstocked[0]
    _dos_names = '、'.join(f'{n}({m:+.1f}%)' for n, m, _ in _declining_overstocked[:3])
    _ai_insights.append(_ins(
        'inv_momentum_divergence', '库存结构', _SEV_H,
        '库存与动销方向相反：跌得最狠的品牌压着最多的钱',
        f'{_dos_names} 环比大跌，却同时是库存资金占用最高的品牌，'
        f'资金沉淀在滞销机型上，既压资金又拖慢周转。',
        [f'{_dos_top[0]} 库存资金 ₦{_dos_top[2]/1e6:.0f}M，环比 {_dos_top[1]:+.1f}%',
         f'91 天以上老库存占比 {_inv_value.get("aging_over90_pct", 0):.1f}%（₦{_inv_value.get("aging_over90_fund", 0)/1e6:.0f}M）'],
        f'优先对 {_dos_top[0]} 及同类滞销品牌做长库龄机型专项清仓（促销/调拨/退换），回笼资金转投热销机型。'))

# ---- 14.3 价量联动：涨价暴跌 或 降价未跑赢大盘 ----
if _price_volume.get('benchmark_pct') is not None:
    _b = _price_volume['benchmark_pct']
    _up = _price_volume.get('increases', {})
    _dn = _price_volume.get('decreases', {})
    _up_cnt = _up.get('count', 0) if isinstance(_up, dict) else 0
    _up_pct = _up.get('avg_price_pct', 0) if isinstance(_up, dict) else 0
    _up_vs = _up.get('avg_post_vs_pre', 0) if isinstance(_up, dict) else 0
    _up_sample = _up.get('sample_post', 0) if isinstance(_up, dict) else 0
    _dn_cnt = _dn.get('count', 0) if isinstance(_dn, dict) else 0
    _dn_vs = _dn.get('avg_post_vs_pre', 0) if isinstance(_dn, dict) else 0
    # 涨价机型调价后销量暴跌（样本≥3 且跌幅显著超过大盘）
    if _up_cnt and _up_vs is not None and _up_sample >= 3 and _up_vs < _b - 20:
        _cand_up = [c for c in _price_volume.get('detail', [])
                    if (c.get('pct') or 0) > 0 and c.get('post_vs_pre') is not None
                    and (c.get('june_sales_qty') or 0) > 0]
        # 优先挑"销量大且跌幅深"的机型(销量权重优先，避免 7 台的小众机型喧宾夺主)
        _worst_up = None
        for _c in _cand_up:
            _c_score = (float(_c.get('june_sales_qty') or 0), -float(_c.get('post_vs_pre') or 0))
            if _worst_up is None or _c_score > _worst_up[1]:
                _worst_up = (_c, _c_score)
        if _worst_up:
            _wu = _worst_up[0]
            _wu_name = f"{_wu.get('brand','')} {_wu.get('model','')}"
            _wu_desc = (f'{_wu.get("brand","")} {_wu.get("model","")}（销量 {_wu.get("june_sales_qty",0):.0f} 台，调价后日均 {_wu.get("post_vs_pre",0):+.1f}%）'
                        if _wu else '个别机型')
        else:
            _wu_name = '个别机型'
            _wu_desc = '个别走量机型'
        _ai_insights.append(_ins(
            'price_increase_impact', '价量归因', _SEV_H,
            f'{_up_cnt} 个机型涨价后销量断崖（{_up_vs:+.1f}%），{_wu_name} 首当其冲',
            f'本期 {_up_cnt} 个机型涨价（均 {_up_pct:+.1f}%），调价后日均 {_up_vs:+.1f}%，'
            f'同期大盘 {_b:+.1f}%，相对大盘落后 {_up_vs - _b:+.1f}pp —— 涨价对走量机型冲击明显。',
            [f'涨价机型调价后日均 {_up_vs:+.1f}%',
             f'同期大盘基准 {_b:+.1f}%',
             f'相对大盘 {_up_vs - _b:+.1f}pp（有效样本 {_up_sample} 个）',
             f'受影响最大：{_wu_desc}'],
            f'对 {_wu_name} 单独复盘：是否存在同价位替代机型、竞品降价、或涨价幅度超出消费者接受区间。'))
    # 降价也未能跑赢大盘 → 需求端问题
    elif _dn_cnt and _dn_vs is not None and _b is not None and _dn_vs < _b:
        _ai_insights.append(_ins(
            'price_demand', '价量归因', _SEV_M,
            '降价也没能跑赢大盘，问题在需求端不在定价',
            f'本轮 {_dn_cnt} 个降价机型调价后日均 {_dn_vs:+.1f}%，同期大盘 {_b:+.1f}%，'
            f'相对大盘落后 {_dn_vs - _b:+.1f}pp —— 降价拉动有限，客流/需求才是主因。',
            [f'降价机型调价后日均 {_dn_vs:+.1f}%',
             f'同期大盘基准 {_b:+.1f}%',
             f'相对大盘 {_dn_vs - _b:+.1f}pp'],
            '不宜继续用降价打价格战，转向提升进店客流、销售转化与促销活动设计；调价样本较小，需再观察 2-3 天。'))

# ---- 14.4 缺货单点风险集中 ----
if _stockout.get('top_models'):
    _so1 = _stockout['top_models'][0]
    _so_total = _stockout['total_lost_units']
    _so1_share = _so1['lost_units'] / _so_total * 100 if _so_total else 0
    if _so1_share >= 30:
        _ai_insights.append(_ins(
            'stockout_concentration', '缺货损失', _SEV_H,
            f'缺货损失四成以上集中在单一型号 {_so1["model"]}',
            f'{_stockout["model_count"]} 个型号存在断货风险，估算损失 {_so_total:.0f} 台 / ₦{_stockout["total_lost_revenue"]/1e6:.1f}M，'
            f'其中 {_so1["brand"]} {_so1["model"]} 单项缺 {_so1["lost_units"]:.0f} 台，占 {_so1_share:.0f}%。',
            [f'{_so1["brand"]} {_so1["model"]} 缺 {_so1["lost_units"]:.0f} 台（占 {_so1_share:.0f}%）',
             f'当前覆盖仅 {_so1["turnover_days"]:.0f} 天'],
            f'立即对 {_so1["model"]} 优先补货/调拨，单点补货即可消除四成缺货损失。'))

# ---- 14.5 门店健康度两极分化 ----
if _health_summary.get('attention') >= 20:
    _att = _health_summary['attention']
    _tot = _health_summary.get('excellent', 0) + _health_summary.get('good', 0) + _health_summary.get('normal', 0) + _att
    _ai_insights.append(_ins(
        'store_health_divergence', '门店健康', _SEV_M,
        f'{_att} 家门店需重点关注，健康度两极分化',
        f'门店健康度评分中「需关注」达 {_att} 家（占 {_att/_tot*100:.0f}%），与「优秀」门店形成明显落差，'
        f'拖累整体达成的正是这批尾部门店。',
        [f'需关注 {_att} 家 / 优秀 {_health_summary.get("excellent", 0)} 家 / 良好 {_health_summary.get("good", 0)} 家',
         f'健康度加权：达成40% / 周转25% / 稳定20% / 人效15%'],
        '对「需关注」门店安排区域经理驻店，按四维指标逐项定位短板（是缺货、周转慢、波动大还是人效低）。'))

# ---- 14.6 高毛利但走量不足的品牌 ----
_hiprofit_lowqty = [b for b in _brand_matrix.get('brands', [])
                    if b['quadrant'] == '高利小众' and b['profit_rate'] >= (_brand_matrix.get('margin_cut', 0) + 2)]
if _hiprofit_lowqty:
    _hp = _hiprofit_lowqty[0]
    _ai_insights.append(_ins(
        'high_margin_underweight', '品牌结构', _SEV_L,
        f'{_hp["brand"]} 毛利率 {_hp["profit_rate"]:.1f}% 属高利，但销量仅 {_hp["qty"]:.0f} 台被低估',
        f'{_hp["brand"]} 毛利率显著高于品牌均值 {_brand_matrix.get("margin_cut", 0):.1f}%，'
        f'但销量落在「高利小众」象限，具备提量空间。',
        [f'{_hp["brand"]} 销量 {_hp["qty"]:.0f} 台 · 毛利率 {_hp["profit_rate"]:.1f}%',
         f'品牌毛利率均值 {_brand_matrix.get("margin_cut", 0):.1f}%'],
        f'评估 {_hp["brand"]} 的铺货与陈列资源，适度加大主推，以高毛利对冲销量缺口。'))

# ---- 14.7 库存资金健康度 ----
_fresh_pct = _inv_value.get('aging', [{}])[0].get('fund_pct', 0) if _inv_value.get('aging') else 0
if _fresh_pct >= 50:
    _ai_insights.append(_ins(
        'inventory_healthy', '库存结构', _SEV_G,
        f'库存资金结构健康，{_fresh_pct:.0f}% 集中在 30 天内新库龄',
        f'门店库存 ₦{_inv_value.get("total_fund", 0)/1e6:.0f}M 中，30 天内库龄占 {_fresh_pct:.0f}%，'
        f'整体库存新鲜度良好，短期无大规模积压风险。',
        [f'30 天内库龄占比 {_fresh_pct:.0f}%',
         f'库存台数 {_inv_value.get("total_stock", 0):.0f} 台'],
        '保持当前进货节奏，持续监控 91 天以上老库存的去化。'))

# 按严重度排序：high > mid > low > good
_SEV_ORDER = {'high': 0, 'mid': 1, 'low': 2, 'good': 3}
_ai_insights.sort(key=lambda x: _SEV_ORDER.get(x['severity'], 9))
_ai_summary = {
    'total': len(_ai_insights),
    'high': sum(1 for i in _ai_insights if i['severity'] == 'high'),
    'mid': sum(1 for i in _ai_insights if i['severity'] == 'mid'),
    'low': sum(1 for i in _ai_insights if i['severity'] == 'low'),
    'good': sum(1 for i in _ai_insights if i['severity'] == 'good'),
    'insights': _ai_insights,
}
print(f"  AI insights: {_ai_summary['total']} 条 (高 {_ai_summary['high']} / 中 {_ai_summary['mid']} / 低 {_ai_summary['low']} / 优 {_ai_summary['good']})")

# ========== MODULE 20: DAILY BRAND & CATEGORY DETAIL (日销售情况) ==========
# 手机口径 = 智能机 + 平板 + 功能机；逐营业日输出品类汇总 + 品牌×日 明细
print("Module 20: Daily brand & category detail...")
PHONE_CATS = [CAT_SMART, CAT_TABLET, CAT_FEATURE]
TRANSSION_BRANDS = ['TECNO', 'INFINIX', 'ITEL']
june_phone = june[june['统计分类'].isin(PHONE_CATS)].copy()
june_phone['品牌'] = june_phone['品牌'].fillna('OTHER').astype(str).str.strip().replace('', 'OTHER')

def _qrp(sub):
    """qty / revenue / profit 三元组"""
    return {
        'qty': int(round(float(sub['销售数量'].sum()))),
        'revenue': int(round(float(sub['零售金额'].sum()))),
        'profit': int(round(float(sub['毛利'].sum()))),
    }

_day_rows = []
for d in sorted(june_phone['记账日期'].dropna().unique()):
    if pd.Timestamp(d).weekday() == 6:  # 周日与 m5 口径一致，跳过
        continue
    dd = june_phone[june_phone['记账日期'] == d]
    row = {
        'date': pd.Timestamp(d).strftime('%Y-%m-%d'),
        'smart': _qrp(dd[dd['统计分类'] == CAT_SMART]),
        'tablet': _qrp(dd[dd['统计分类'] == CAT_TABLET]),
        'feature': _qrp(dd[dd['统计分类'] == CAT_FEATURE]),
        'total': _qrp(dd),
    }
    _tsmart = dd[(dd['统计分类'] == CAT_SMART) & (dd['品牌'].isin(TRANSSION_BRANDS))]['销售数量'].sum()
    _sqt = dd[dd['统计分类'] == CAT_SMART]['销售数量'].sum()
    row['tsmart_share'] = round(float(_tsmart) / float(_sqt) * 100, 1) if _sqt > 0 else 0
    _day_rows.append(row)

_bd = june_phone.groupby([june_phone['记账日期'].dt.strftime('%Y-%m-%d'), '品牌']).agg(
    qty=('销售数量', 'sum'), revenue=('零售金额', 'sum'), profit=('毛利', 'sum')).reset_index()
brand_daily = [{'date': r.iloc[0], 'brand': r.iloc[1], 'qty': int(round(float(r.iloc[2]))),
                'revenue': int(round(float(r.iloc[3]))), 'profit': int(round(float(r.iloc[4])))}
               for _, r in _bd.iterrows()]
_daily_detail = {'days': _day_rows, 'brand_daily': brand_daily}
print(f"  Daily detail: {len(_day_rows)} days, {len(brand_daily)} brand-day rows")

# ========== MODULE 21: MONTH BRAND & CATEGORY DETAIL (月度完成情况) ==========
# 本期(月初~报告日) vs 上月同期(相同营业日数)，手机口径
print("Module 21: Month brand & category detail...")
may_phone = may_full[may_full['统计分类'].isin(PHONE_CATS)].copy()
may_phone_cmp = may_phone[may_phone['记账日期'].isin(may_biz_compare)].copy()
may_phone_cmp['品牌'] = may_phone_cmp['品牌'].fillna('OTHER').astype(str).str.strip().replace('', 'OTHER')

def _phone_block(df):
    brands = []
    for b, g in df.groupby('品牌'):
        q = float(g['销售数量'].sum())
        if q <= 0:
            continue
        brands.append({
            'brand': b, 'qty': int(round(q)),
            'revenue': int(round(float(g['零售金额'].sum()))),
            'profit': int(round(float(g['毛利'].sum()))),
        })
    brands.sort(key=lambda x: x['qty'], reverse=True)
    cats = {
        'smart': _qrp(df[df['统计分类'] == CAT_SMART]),
        'tablet': _qrp(df[df['统计分类'] == CAT_TABLET]),
        'feature': _qrp(df[df['统计分类'] == CAT_FEATURE]),
    }
    return {'brands': brands, 'cats': cats, 'total': _qrp(df)}

_prev_label = (f"{may_biz_compare[0].strftime('%Y/%m/%d')}-{may_biz_compare[-1].strftime('%m/%d')}"
               f" · {len(may_biz_compare)}营业日")
_month_detail = {
    'cur': _phone_block(june_phone),
    'prev': _phone_block(may_phone_cmp),
    'prev_label': _prev_label,
    'elapsed_days': ELAPSED_DAYS,
}
print(f"  Month detail: cur {len(_month_detail['cur']['brands'])} brands, prev {len(_month_detail['prev']['brands'])} brands | vs {_prev_label}")
...
output = {
    'meta': {
        ...
        # 手机(智能机+平板)口径营收/毛利 — 与智能机销量同一口径，避免与全品类口径混淆
        'total_smart_revenue': round(june_smart_combo['零售金额'].sum(), 0),
        'total_smart_profit': round(june_smart_combo['毛利'].sum(), 0),
    ...
    'm2_store_category': store_cat,          # 全量门店(不截断)
    'cat_structure': cat_structure,
    'm3_brands': june_brands.to_dict('records'),
    'm4_daily_detail': store_daily,          # 全量门店(不截断)
    ...
    # ===== Director cockpit (Module 13) =====
    'm13_forecast': _forecast,
    'm14_inv_value': _inv_value,
    'm15_price_volume': _price_volume,
    'm16_store_health': _health_summary,
    'm17_stockout': _stockout,
    'm18_brand_matrix': _brand_matrix,
    'm19_ai_insights': _ai_summary,
    # ===== 日销售情况 / 月度完成情况 (Module 20/21) =====
    'm20_daily_detail': _daily_detail,
    'm21_month_detail': _month_detail,
}
